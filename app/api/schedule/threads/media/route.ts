...
function validateFileMeta(file: UploadRequestFile) {
  ...
  if (!type) {
    return { error: `${file.name || 'file'}は対応していないファイル形式です` };
  }
  const size = typeof file.size === 'number' ? file.size : 0;
  const maxBytes = type === 'VIDEO' ? MAX_VIDEO_BYTES : MAX_IMAGE_BYTES;
  if (size > maxBytes) {
    return { error: `${file.name || 'file'}のサイズが大きすぎます` };
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (contentType.includes('application/json')) {
      ...
      for (let index = 0; index < files.length; index++) {
        ...
        const signed = await createSignedGCSUploadUrl(
          ...
          fileName,
          `threads/scheduled/${userId}/${group}`,
        );
        if (!signed) {
    ...
    console.error('[schedule/threads/media] POST failed', error);
