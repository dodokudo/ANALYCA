...
export async function POST(request: NextRequest) {
  try {
    ...
    if (subStatus.pending_plan_id || subStatus.pending_subscription_id) {
      return NextResponse.json(
        { success: false, error: '予約済みのプラン変更があります。サポートへお問い合わせください' },
        { status: 400 },
      );
    }
    ...
    try {
      ...
      await updateSubscription(subStatus.subscription_id, {
        ...
        next_payment: { amount: currentAmount },
        metadata: {
          ...(subscription.metadata || {}),
    ...
    try {
      ...
      console.error('Failed to sync upgraded subscription to LINE Harness:', syncError);
    }
    ...
    return NextResponse.json({
      ...
      planId: resolvedTargetPlanId,
      diffAmount,
      message: `${targetPlan.name}プランへ更新し、差額 ${diffAmount.toLocaleString('ja-JP')}円 を決済しました`,
    });
  } catch (error) {
