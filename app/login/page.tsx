...
export default function LoginPage() {
  ...
  const handleThreadsLogin = () => {
    ...
    window.location.href = `https://threads.com/oauth/authorize?client_id=${clientId}&redirect_uri=${redirectUri}&scope=${scope}&response_type=code`;
  };
      ...
      {showInstagramModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 py-6 overflow-y-auto">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 my-auto">
            ...
            >
              {isInstagramLoading ? 'リダイレクト中...' : 'ログインに進む'}
            </button>
          </div>
        </div>
      )}
