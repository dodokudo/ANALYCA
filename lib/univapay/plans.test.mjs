...
test('keeps approved yearly prices and monthly equivalents', () => {
  ...
  assert.equal(PLANS['light-instagram-yearly'].price, 47_760);
  assert.equal(getMonthlyEquivalentPrice('light-instagram-yearly'), 3_980);
...
test('keeps the Instagram-only checkout at 4,980 yen with Instagram onboarding', () => {
  assert.equal(PLANS['light-instagram'].price, 4_980);
  assert.equal(PLANS['light-instagram'].onboardingPath, '/onboarding/light2');
  assert.equal(PLANS['light-instagram-yearly'].onboardingPath, '/onboarding/light2');
});
...
test('builds monthly and yearly plan ids', () => {
  ...
  assert.equal(getPlanIdForBillingCycle('light-instagram', 'monthly'), 'light-instagram');
  assert.equal(getPlanIdForBillingCycle('light-instagram', 'yearly'), 'light-instagram-yearly');
});
